...
const RelatedPostsList = props => {
  const renderTagList = () => {
    const { tags_post } = props.relatedpost;
    return(
  ...
  const { id, title, featured_image_src, slug, excerpt, author_name, author_avatar, published_date, views, love_it, comments_number } = props.relatedpost;
  return(
    <div className="col">
      <article id={`post-${id}`} className="col-posts">
        {featured_image_src && 
        <Link to={slug} onClick={handlePostView}>
        ...
        <div className="entry-post-list">
          ...
          {title && <h3><Link to={slug} onClick={handlePostView}>{title}</Link></h3>}
