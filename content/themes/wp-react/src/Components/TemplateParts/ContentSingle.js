...
const ContentSingle = props => {
  const handleLoveIt = async () => {
    ...
    try {
      const response = await fetch(WPReactSettings.URL.api + "/loves/" + id, {
        method: 'POST'
      });
      if(response.ok){
      ...
      <article id={`post-${id}`} className="single">
        <div className="cntr">
          <div className="meta">
            ...
            <button id="btn-love-it" onClick={handleLoveIt}>
  ...
  const { id, next_post, previous_post, related_posts } = props.post;
  return(
