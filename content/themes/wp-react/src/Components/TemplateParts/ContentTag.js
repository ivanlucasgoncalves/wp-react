...
const ContentTag = props => {
  ...
  const { id, title, featured_image_src, slug, excerpt, author_name, author_avatar, published_date, views, love_it, comments_number } = props.tag;
  return(
    <div className="col">
      <article id={`post-${id}`} className="col-posts">
        ...
        <div className="entry-post-list">
          ...
          {title && <h3><Link to={`/blog/${slug}`} onClick={handlePostView}>{title}</Link></h3>}
